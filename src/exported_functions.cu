...
extern "C" void nerf_update_texture(
	float *camera_matrix0,
	unsigned int handle,
	...
	int is_rgb = 1)
{
	...
	Eigen::Matrix<float, 3, 4> camera0{camera_matrix0};
	Eigen::Matrix<float, 3, 4> camera1;
	if (camera_matrix1 == 0)
	{
		camera1 = camera0;
	}
	...
	{
		camera1 = Eigen::Matrix<float, 3, 4>{camera_matrix1};
	}

	Eigen::Vector4f nerf_rolling_shutter;
	if (rolling_shutter == 0)
	{
		nerf_rolling_shutter = Eigen::Vector4f::Zero();
	}
	else
	{
		nerf_rolling_shutter = Eigen::Vector4f{rolling_shutter};
	}

	RenderBuffer render_buffer = found->second->render_buffer;
	render_buffer->reset_accumulation();
	testbed->render_frame(camera0,
						  camera1,
...
extern "C" Eigen::Vector2f nerf_fov_xy()
{
	if (!testbed)
		return;

	return testbed->fov_xy();
}

extern "C" void nerf_set_fov_xy(float *xy)
{
	if (!testbed)
		return;

	Eigen::Vector2f fov_xy{xy};
	testbed->set_fov_xy(fov_xy);
}
